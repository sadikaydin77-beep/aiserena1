...
def call_claude(prompt):
    r = requests.post("https://api.anthropic.com/v1/messages",
        ...
        json={"model":"claude-sonnet-4-20250514","max_tokens":500,"messages":[{"role":"user","content":prompt}]},timeout=60)
    data = r.json()
...
def generate_image(prompt):
    r = requests.post("https://api.openai.com/v1/images/generations",
        ...
        json={"model":"gpt-image-1","prompt":prompt,"size":"1024x1024","quality":"high","n":1},timeout=120)
    data = r.json()
    if "data" not in data:
