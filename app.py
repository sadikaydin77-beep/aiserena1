...
def generate_image_prompt(trend):
    return call_claude(f"Write a gpt-image-1 prompt for a luxury minimalist jewelry product photo. Theme: {trend['keyword']}. Style: {trend['style']}. Colors: {trend['palette']}. Rules: professional product photography, clean background white marble or cream linen, natural soft light, high-end brand feel, max 2 jewelry pieces, no hands, no models, no people. Return only the prompt text under 100 words.")
...
def generate_caption(trend):
    raw = call_claude(f"Write an Instagram caption for AiSerena, a minimalist fine jewelry brand. Theme: {trend['keyword']}. 2-3 sentences in English, elegant tone, 3-4 emojis, end with CTA like 'Shop the collection, link in bio'. Add 20 hashtags. Return ONLY valid JSON: {{\"caption\": \"...\", \"hashtags\": \"#... #...\"}}")
...
def send_telegram(content_id, image_url, caption, hashtags):
    ...
    if image_url.startswith("data:"):
        requests.post(
            ...
            json={"chat_id": TELEGRAM_CHAT_ID, "text": f"New post ready!\n\n{caption}\n\n{hashtags}\n\n[Image generated]", "reply_markup": keyboard}
        )
    else:
        requests.post(
            f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendPhoto",
            json={"chat_id": TELEGRAM_CHAT_ID, "photo": image_url, "caption": f"New post ready!\n\n{caption}\n\n{hashtags}", "reply_markup": keyboard}
        )
